...
# равна числу m и наоборот. Например, 220 и 284 – дружественные 
# числа. По данному числу k выведите все пары дружественных чисел, 
# каждое из которых не превосходит k. Программа получает на вход 
# одно натуральное число k, не превосходящее 105. 
# Программа должна вывести все пары дружественных чисел, каждое 
# из которых не превосходит k. Пары необходимо выводить по одной 
# в строке, разделяя пробелами. Каждая пара должна быть выведена 
...
def pair(value):
    ...
    # Иначе прибавляем это число (т.е. корень), т.к. оно тоже множитель
    ...
    for i in range(2, sq_num):
        ...
        if value % i == 0:
            ...
            # Складываем в переменную значение i и второй делитель, 
            # например value = 10, i = 2, второй делитель 5
            res += i + value // i
    return res
...
for j in range(10, 300):
    ...
    # Второе условие защита от дубликатов, записанных наоборот
